...
package alternativa.engine3d.resources {
	...
	public class Geometry extends Resource {
		...
		public function calculateTangents(uvChannel:int):void {
			...
			var numIndices:int = _indices.length;
			//trace("Num indices = "+numIndices.toString());
			var normal:Vector3D;
			var tangent:Vector3D;
			var i:int;
			
			if (tangents.length<_numVertices)
			{
				for(i = 0; i < _numVertices; i++)
				{
					if(tangents[i]==null)
					{
						tangents[i]=new Vector3D(1,0,0,1);
					}
				}
			}			
			...
			/*if (tangents.length<_numVertices)
			{
				var raz:int = _numVertices-tangents.length;
				//var tl:int = tangents.length;
				for(i = 0; i < _numVertices; i++)
				{
					if(tangents[i]==null)
					{
						tangents[i]=new Vector3D();
					}
				}
			}*/
			/*for(i = 0; i < _numVertices; i++)
			{
				var v:Vector3D = tangents[i];
				trace("vertex["+i.toString()+"] = "+v.toString());
					
			}*/

			//trace("bufer length = "+_numVertices.toString()+", tanget length = "+tangents.length);
		 ...
		 * @inheritDoc
		 */
		override public function upload(context3D:Context3D=null):void {
			if(context3D==null)
			{
				context3D=Helpers.CONTEXT;
			}
