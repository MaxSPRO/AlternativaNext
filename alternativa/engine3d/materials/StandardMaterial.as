...
//		private static const OBJECT_TYPE_BIT:int = 19;
		 ...
		 * @private
		 */
		alternativa3d var fogMode:int = DISABLED;
		/**
		 * @private
		 */
		alternativa3d var fogNear:Number = 1000;
		/**
		 * @private
		 */
		alternativa3d var fogFar:Number = 5000;

		/**
		 * @private
		 */
		alternativa3d var fogMaxDensity:Number = 1;

		/**
		 * @private
		 */
		alternativa3d var fogColorR:Number = 0xC8/255;
		/**
		 * @private
		 */
		alternativa3d var fogColorG:Number = 0xA2/255;
		/**
		 * @private
		 */
		alternativa3d var fogColorB:Number = 0xC8/255;

		/**
		 * @private
		 */
		alternativa3d var fogTexture:TextureResource;
		...
		private static const _setGlossinessFromTextureProcedure:Procedure = new Procedure([
			...
			"#c0=cSurface",
			"#s0=sGlossiness",
			"tex t0, v0, s0 <2d, "+Helpers.COMPRESS+", repeat, linear, miplinear>",
			"mul o0.w, t0.x, c0.y"
		...
		private static const _getNormalAndViewTangentProcedure:Procedure = new Procedure([
			...
			"tex t0, v3, s0 <2d, "+Helpers.COMPRESS+",repeat,linear,miplinear>",
			"add t0, t0, t0",
		...
		private static const outputWithAdvancedFogProcedure:Procedure = new Procedure([
			...
			"mul t0.z, t0.z, c0.x",
			"add t1.x, t1.x, t0.z",
			"tex t1, t1, s0 <2d, "+Helpers.COMPRESS+", repeat, linear, miplinear>",
		 ...
		 */
		public var specularPower:Number = 0.45;
		
		public var makeShadows:Boolean = true;
		...
		public function StandardMaterial(diffuseMap:TextureResource = null, normalMap:TextureResource = null, specularMap:TextureResource = null, glossinessMap:TextureResource = null, opacityMap:TextureResource = null) {
			...
			if(Helpers.fogParam!==null)
			{
				this.fogFar = Helpers.fogParam.fogFar;
				this.fogNear = Helpers.fogParam.fogNear;
				this.fogMode = Helpers.fogParam.fogMode;
				this.fogColorB = Helpers.fogParam.fogColorB;
				this.fogColorG = Helpers.fogParam.fogColorG;
				this.fogColorR = Helpers.fogParam.fogColorR;				
				this.fogMaxDensity = Helpers.fogParam.fogMaxDensity;
				this.fogTexture = Helpers.fogParam.fogTexture;				
			}
		}
...
//			fragmentLinker.setInputParams(omniMulShadowProcedure, "tNormal", "tViewVector", "tTotalLight", "cAmbientColor");
		...
		private function getProgram(object:Object3D, programs:Array, camera:Camera3D, materialKey:int, opacityMap:TextureResource, alphaTest:int, lightsGroup:Vector.<Light3D>, lightsLength:int, isFirstGroup:Boolean, shadowedLight:Light3D):StandardMaterialProgram {
			...
			if (program == null) {
				...
				if (lightsLength > 0 || shadowedLight) {
					...
					if (shadowedLight != null || makeShadows) {
		...
		private function addDrawUnits(program:StandardMaterialProgram, camera:Camera3D, surface:Surface, geometry:Geometry, opacityMap:TextureResource, lights:Vector.<Light3D>, lightsLength:int, isFirstGroup:Boolean, shadowedLight:Light3D, opaqueOption:Boolean, transparentOption:Boolean, objectRenderPriority:int):void {
			...
			if(makeShadows || light.exclude(object))
			{
